...
if __name__ == '__main__':
    ...
    std_noise_p = add_noise * 0.01
    std_noise_theta = add_noise * 0.001
    ...
    y_meas = np.copy(y) + add_noise*np.random.randn(*y.shape) * std_noise
    Ts = np.float(t[1] - t[0])
    ...
    taps = signal.remez(numtaps, [0, cutoff, cutoff + trans_width, 0.5 * fs], [2 * np.pi * cutoff * 10 * 1.5, 0], Hz=fs,
                        type='differentiator')
